...
def conexion_db():
    try:
        mibase = sqlite3.connect('riesgos_dbv.db')   
        return mibase
    except:
...
def crear_tablas():
    try:
        ...
        micursor.execute("CREATE TABLE compania(id INTEGER PRIMARY KEY AUTOINCREMENT,nombre_cia TEXT NOT NULL);")
        micursor.execute("CREATE TABLE activo(id INTEGER PRIMARY KEY AUTOINCREMENT,nombre_act TEXT NOT NULL);")
        micursor.execute("CREATE TABLE vulne(id INTEGER PRIMARY KEY AUTOINCREMENT,nombre_vul TEXT NOT NULL);")
        micursor.execute("CREATE TABLE amenazas(id INTEGER PRIMARY KEY AUTOINCREMENT,nombre_ame TEXT NOT NULL);")
        micursor.execute("CREATE TABLE relaciones(id_rel INTEGER PRIMARY KEY AUTOINCREMENT,id_act INTEGER,"
                         "nombre_act TEXT,id_vul INTEGER,nombre_vul TEXT,id_ame INTEGER,nombre_ame TEXT);")
        micursor.execute("CREATE TABLE riesgos(id INTEGER PRIMARY KEY AUTOINCREMENT,nombre_cia TEXT,nombre_act TEXT,"
                                               "nombre_vul TEXT,nombre_ame TEXT, probabilidad INTEGER,"
                                               "intensidad INTEGER,riesgo INTEGER);")
        mibase.commit() 
    except:
...
def grafico(compania):
    ...
    conexion = conexion_db()
    micursor = conexion.cursor()
    sql="SELECT probabilidad,intensidad FROM riesgos WHERE nombre_cia =?;"
    ...
    a=0
    xy_static1,xy_static2,xy_linea=[],[],[]
    while a <4: #grafico lineas de cuadrantes
        xy_static1.append(1)
        xy_static2.append(2)
        xy_linea.append(a) 
        a+=1
    ...
    ax.plot(xy_static1,xy_linea, color='black')
    ax.plot(xy_static2,xy_linea, color='black')
    ax.plot(xy_linea,xy_static1, color='black')
    ax.plot(xy_linea,xy_static2, color='black')   
    plt.show()
    # Mostrar
    ...
    try:
        ...
        if askyesno("Actualización de Datos Maestros", "Desea dar {0} en {1} ?".format(tipo,tabla)):
            if tipo=="alta": #podria simplicar en un solo Query si modificara el nombre de los campos de las tablas, no se si es buena practica..
                if tabla=="compania":
                    sql = "INSERT INTO compania (nombre_cia) VALUES (?);"
                elif tabla=="amenazas":
                    sql = "INSERT INTO amenazas (nombre_ame) VALUES (?);"
                elif tabla=="activo":
                    sql = "INSERT INTO activo (nombre_act) VALUES (?);"
                elif tabla=="vulnerabilidades":
                    sql = "INSERT INTO vulne (nombre_vul) VALUES (?);"
                elif tabla=="relacion":
                     sql ="INSERT INTO relaciones (nombre_ame,id_ame,nombre_vul,id_vul,nombre_act,id_act) VALUES (?,?,?,?,?,?);"
                elif tabla=="riesgo":
                    sql ="INSERT INTO riesgos (nombre_cia,nombre_act,nombre_vul,nombre_ame,intensidad,probabilidad,riesgo) VALUES (?,?,?,?,?,?,?);"
            elif tipo=="baja":
                if tabla=="compania":
                    sql = "DELETE FROM compania WHERE nombre_cia = (?);"
                elif tabla=="amenazas":
                    sql = "DELETE FROM amenazas WHERE nombre_ame= (?);"
                elif tabla=="activo":
                    sql = "DELETE FROM activo WHERE nombre_act = (?);"
                elif tabla=="vulnerabilidades":
                    sql = "DELETE FROM vulne WHERE nombre_vul = (?);"  
                elif tabla=="relacion":
                    sql = "DELETE FROM relaciones WHERE id_rel = (?);"
                elif tabla=="riesgos":
                    sql = "DELETE FROM riesgos WHERE id = (?);"
            elif tipo=="modificar":
                if tabla=="riesgos":
                    sql = "UPDATE riesgos SET probabilidad=?, intensidad=?, riesgo=?  WHERE id = (?);"
            micursor.execute(sql,ingresar) 
            conexion.commit()
            entrada.set("")
